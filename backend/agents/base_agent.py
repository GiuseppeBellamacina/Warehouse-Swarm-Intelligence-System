...
class BaseAgent(Agent):
    ...
    def perceive_environment(self) -> List[Tuple[int, int, CellType]]:
        ...
        pos_tuple = pos_to_tuple(self.pos)
        visible = self.model.grid.get_visible_cells(
            pos_tuple[0],
            pos_tuple[1],
            self.vision_radius
        )
    ...
    def get_nearby_agents(self, radius: Optional[float] = None) -> List[Agent]:
        ...
        pos_tuple = pos_to_tuple(self.pos)
        agent_indices = self.model.grid.get_agents_in_radius(
            pos_tuple[0],
            pos_tuple[1],
            radius
        )
    ...
    def _check_collision(self, new_pos: Tuple[int, int]) -> bool:
        ...
        """
        # Get current agents at target position
        cell_contents = self.model.grid.get_cell_list_contents([new_pos])
        
        # For group mode: no overlap allowed (except in extreme cases)
        # Allow coordinators to have priority
        if cell_contents:
            # If there's already an agent, only allow if this is coordinator
            if self.role == "coordinator":
                return True
            # Or if we're at warehouse (crowding allowed)
            cell_type = self.model.grid.get_cell_type(*new_pos)
            if cell_type in [CellType.WAREHOUSE, CellType.WAREHOUSE_ENTRANCE]:
                return True
            return False
    ...
    def step_sense(self) -> None:
        ...
        self.consume_energy(self.energy_consumption['base'])
